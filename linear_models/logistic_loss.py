...
######### Loss
L_theta = (1/n) * torch.log(1+torch.exp(-Delta * hat_Y)).sum() + 0.5*gamma*torch.trace(V.T@V)
...
######### Gradient Y
dhat_Y = -(1/n) * Delta * (1-torch.sigmoid(Delta * hat_Y))  # (n, n)
...
######### Hessian Y
tmp = torch.sigmoid(Delta*hat_Y).squeeze() # (n, 1)
Hess_Y = (1/n)*torch.diag(tmp * (1-tmp)) # (n, n)
